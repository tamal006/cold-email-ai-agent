# DEPLOYMENT GUIDE & DEVELOPER ONBOARDING: ColdMail AI Agent
...
### Prerequisites
- **Node.js**: Version 18.x or higher (Node 24+ recommended for native file watching).
...
- **Gmail Account**: Required for Nodemailer email delivery (uses App Passwords).
...
4. **Running Locally in Development**
   ...
   - **Frontend**:
     ...
     npm run dev
     ```
     This starts the Vite development server at `http://localhost:5173`. Open your browser to verify it runs.
...
| `MONGODB_URI` | Yes | `mongodb://localhost:27017/coldmail` | MongoDB connection URI |
...
| `FROM_NAME` | No | `ColdMail AI Agent` | Sender name displayed to recipients |
...
4. Copy the connection string (e.g. `mongodb+srv://<username>:<password>@cluster0.abcde.mongodb.net/coldmail-agent?retryWrites=true&w=majority`) and paste it as `MONGODB_URI` in `backend/.env`.
...
4. Select **Mail** as the app and **Other (Custom Name)** as the device (e.g., "ColdMail Agent").
...
- [ ] **Check SMTP Deliverability**: Test the Nodemailer configuration using the health check panel before deploying.
